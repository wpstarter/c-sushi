...
trait Sushi
{
    ...
    public static function bootSushi()
    {
        ...
        $cacheFileName = ws_config('sushi.cache-prefix', 'sushi').'-'.Str::kebab(str_replace('\\', '', static::class)).'.sqlite';
        $cacheDirectory = realpath(ws_config('sushi.cache-path', ws_storage_path('framework/cache')));
    ...
    {
        /** @var \WpStarter\Database\Schema\SQLiteBuilder $schemaBuilder */
        ...
        try {
            ...
            if (Str::contains($e->getMessage(), 'already exists (SQL: create table')) {
                // This error can hws_appen in rare circumstances due to a race condition.
